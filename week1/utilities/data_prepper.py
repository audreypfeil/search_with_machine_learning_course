...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # IMPLEMENT_START --
        # print("maybe implemented IMPLEMENT ME: __log_ltr_query_features: Extract log features out of the LTR:EXT response and place in a data frame")
        ...
        for doc in response["hits"]["hits"]:
            ...
            for features in doc["fields"]["_ltrlog"][0]["log_entry"]:
                features.update({"doc_id": doc["_id"]})
                add_features.append(features)                   
                
        feature_df = pd.DataFrame(data=add_features, columns=['name', 'value', 'doc_id']).fillna(0).reset_index()
        pivot_df = feature_df.pivot(index='doc_id', columns=['name'], values='value').reset_index()
        pivot_df['doc_id'] = pivot_df['doc_id'].astype(int)

        frame = pd.DataFrame(feature_results)
        frame = pd.merge(frame, pivot_df, on='doc_id', how='outer')

        frame.to_html('temp.html')
        print("df")
        print(frame)
        print("log_entry")
        print(response["hits"]["hits"][0]["fields"])
